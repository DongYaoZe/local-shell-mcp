...
Use local-shell-mcp. In `/workspace/FrameDiff`, create a new branch `ai/example-change`, make the requested code edit with `file_edit` or `file_patch`, run relevant tests, show `git diff --stat`, run `secret_scan`, commit, and push the branch.
...
Use local-shell-mcp. With `run_shell` and `machine` set to `npu-4card`, run `pwd`, `hostname`, `python3 --version`, `git log -1 --oneline`, and `npu-smi info` from `/home/cyh/FrameDiff`.

## Remote code edit and test

Use local-shell-mcp. On remote machine `hpc-a`, inspect `/home/cyh/project` with `file_tree`, search with `file_grep`, edit with `file_edit` or `file_patch`, and run the relevant test plus `git diff` with `run_shell`. Set `machine` to `hpc-a` on every call.


## Remote host-to-host file transfer

Use local-shell-mcp. Copy `/data/run-42/result.tar.zst` from remote machine `hpc-a` to `/scratch/imports/result.tar.zst` on remote machine `hpc-b` using `remote_transfer` with both machine endpoints, then verify the destination size and checksum with `run_shell` on `hpc-b`.
