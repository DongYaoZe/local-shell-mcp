...
倉庫只提供一份可直接複製的起始配置：[`.env.example`](https://github.com/fwerkor/local-shell-mcp/blob/main/.env.example)。Docker Compose 會自動讀取複製後的 `.env`，其他運行方式也可以使用相同的 `LOCAL_SHELL_MCP_` 環境變量。YAML 僅作為二進制或源碼部署的可選高級輸入；需要時自行創建文件，並通過 `LOCAL_SHELL_MCP_CONFIG` 或 `--config` 顯式指定。環境變量會覆蓋 YAML 值，因此除非有意覆蓋，否則不要在兩處重複定義同一設置。YAML key 使用下表中的字段名。
...
| `max_skill_scan_entries` | `LOCAL_SHELL_MCP_MAX_SKILL_SCAN_ENTRIES` | `5000` | 單次 `skill_list` 註冊表掃描或單次指定 Skill 載入最多檢查的檔案系統條目數。 |
...
| `max_http_request_bytes` | `LOCAL_SHELL_MCP_MAX_HTTP_REQUEST_BYTES` | `16000000` | MCP、REST、OAuth、UI 與遠端 worker 端點允許緩衝的最大 HTTP 請求體位元組數。 |
| `max_job_log_bytes` | `LOCAL_SHELL_MCP_MAX_JOB_LOG_BYTES` | `10000000` | 每次長任務運行保留的最大輸出字節數。 |
| `max_jobs` | `LOCAL_SHELL_MCP_MAX_JOBS` | `1000` | 最多保留的長任務記錄數；執行中的任務不會被清理。 |
...
| `require_auth_for_mcp_discovery` | `LOCAL_SHELL_MCP_REQUIRE_AUTH_FOR_MCP_DISCOVERY` | `True` | MCP 初始化與工具發現是否要求 OAuth 認證。 |
| `mcp_session_idle_timeout_s` | `LOCAL_SHELL_MCP_MCP_SESSION_IDLE_TIMEOUT_S` | `180` | Stateful Streamable HTTP 會話空閒超時。 |
| `mcp_max_sessions` | `LOCAL_SHELL_MCP_MCP_MAX_SESSIONS` | `1024` | Stateful MCP 會話最大併發數。 |
