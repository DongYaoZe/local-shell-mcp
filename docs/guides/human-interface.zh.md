...
当 ChatGPT 支持渲染 MCP Apps 时，`workspace_open(session_id=...)` 会为**明确指定的 Logical Session** 打开悬浮式协作视图。Session 持有持久任务状态——objective、progress、Plan 和 Activity；Live Workspace 只负责展示这些状态、实时活动和人类控制。它不会从 MCP transport 推断任务身份。
