...
للأرشيفات المضغوطة حد مستقل عبر `LOCAL_SHELL_MCP_MAX_AUDIT_ARCHIVE_BYTES` وقيمته الافتراضية 512 MB. عند تجاوزه تُحذف أقدم الأرشيفات أولًا. يمكن ضبطه على `0` لتعطيل الاحتفاظ المضغوط طويل الأجل. تقرأ Web UI واستعلامات Activity/Audit و`audit_tail` سجل hot log النشط فقط. وتُستخدم الأرشيفات المضغوطة كتخزين بارد للاحتفاظ أو التصدير، ولا تفك استعلامات UI العادية ضغطها تلقائيًا.
